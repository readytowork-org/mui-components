...
> extends UseClearableFieldSlotProps {
  ...
  fieldSeparator?: SlotComponentProps<
    typeof Typography,
    {},
    Record<string, any>
  >;
...
>) => {
  ...
  const handleStartFieldRef = useForkRef(
    fieldProps.unstableStartFieldRef,
    startFieldRef,
  );
  const handleEndFieldRef = useForkRef(
    fieldProps.unstableEndFieldRef,
    endFieldRef,
  );
  ...
  React.useEffect(() => {
    ...
    const currentFieldRef =
      rangePosition === "start" ? startFieldRef : endFieldRef;
  ...
  const slotProps: ReturnType["slotProps"] & {
    ...
    textField: (ownerState) => {
      const resolvedComponentProps = resolveComponentProps(
...
>) => {
  ...
  React.useEffect(() => {
    ...
    if (currentView) {
      const sections = startFieldRef.current
        .getSections()
  ...
  const updateRangePosition = () => {
    ...
    const domRangePosition =
      activeSectionIndex == null || activeSectionIndex < sections.length / 2
        ? "start"
        : "end";
